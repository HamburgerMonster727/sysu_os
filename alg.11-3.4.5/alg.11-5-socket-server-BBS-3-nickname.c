...
{
    switch (stat) {
    case 0: strcpy(stat_str, "STAT_EMPTY");
        break;
    case 1: strcpy(stat_str, "STAT_ACCEPTD");
        break;
    case 2: strcpy(stat_str, "STAT_NORMAL");
        break;
    case -1: strcpy(stat_str, "STAT_ENDED");
        break; 
    default: strcpy(stat_str, "STAT_UNKNOWN");
        break;
    }
    ...
    while(1) { 
        ...
        while (1) { /* update sn_stat and nickname from recv_send_data(), or brocast msg to all sn */
            ...
            if(msg_buf[4] == '#') {
                ...
                if(msg_buf[5] == '2') { /* #1: listing all connectors of the BBS */
                    memset(msg_buf, 0, MSG_SIZE);
                    sprintf(msg_buf, "\n====   connector list   =====");
                    ret = write(fd[sn][1], msg_buf, MSG_SIZE); /* non-blocking write ord-pipe */
                    if(ret <= 0) {
        ...
        while (1) { /* read from input terminal and brocast to all sn */
            ...
            else if(stdin_buf[0] == '#') { /* #: listing all connectors of the BBS */
                printf("\n====   connector list   =====\n");
                for (i=0; i<=max_sn; i++) { /* sn_attri[0].nickname = "Console" */
